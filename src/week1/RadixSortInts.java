...
public class RadixSortInts {
    ...
    public static void radixSort(int[] arr){
        ...
        int n=0, p=0;
        // build negative and positive arrays:
        for (int t : arr) {
            if (t < 0) {
                negativeArr[n++] = -t;
            } else {
        ...
        // resize arrays to fit (remove extra zeroes):
        int[] temp = new int[n];
        for(int k=0; k<n; k++){
        ...
        // find max of arrays for the sort:
        int maxNegative = max(negativeArr),
                maxPositive = max(positiveArr);
        // counting sort on each of the digits; this is the 'radix' part of the sort.
        ...
        // rebuild array from negative and positive arrays:
        int i=0, j=0;
        while (i<n){
